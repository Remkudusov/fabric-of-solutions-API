...
class AdminPollView(APIView):
    ...
    def post(self, request, action, poll_id=None, *args, **kwargs):
        if action == "post":
            serializer = serializers.PostPollSerializer(data=request.data, many=True)
        ...
        if poll_id != None:
            if action == "update":
                serializer = serializers.UpdatePollSerializer(data=request.data)
                if serializer.is_valid():
                    poll = models.Poll.objects.get(id=poll_id)
                    setattr(poll, serializer.data['field'], serializer.data['value'])
                    poll.save()
                    return Response(serializer.data)
                return Response(serializer.errors)
            if action == "delete":
                models.Poll.objects.get(id=poll_id).delete()
                data = {
...
class AdminQuestionView(APIView):
    ...
    def post(self, request, action, question_id=None, *args, **kwargs):
        if action == "post":
            serializer = serializers.PostQuestionSerializer(data=request.data, many=True)
            if serializer.is_valid():
                serializer.save()
                return Response(serializer.data)
            return Response(serializer.errors)
        if question_id != None:
            if action == "update":
                serializer = serializers.UpdateQuestionSerializer(data=request.data)
                if serializer.is_valid():
...
class AdminVariantView(APIView):
    ...
    def post(self, request, action, question_id=None, variant_number=None):
        if action == "post":
            serializer = serializers.PostVariantSerializer(data=request.data, many=True)
            if serializer.is_valid():
                serializer.save()
                return Response(serializer.data)
            return Response(serializer.errors)
        if question_id != None and variant_number != None:
            if action == "update":
                serializer = serializers.UpdateVariantSerializer(data=request.data)
                if serializer.is_valid():
                    variant = models.Variant.objects.get(question_id=question_id, number=variant_number)
                    setattr(variant, serializer.data['field'], serializer.data['value'])
                    variant.save()
                    return Response(serializer.data)
                return Response(serializer.errors)
            if action == "delete":
                models.Variant.objects.get(question_id=question_id, number=variant_number).delete()
                data = {
...
class PollsView(APIView):
    def get(self, request, subject, *args, **kwargs):
        if subject == "polls":
            ...
            serializer = serializers.GetPollSerializer(actual_polls, many=True)
            return Response(serializer.data)
...
class AnswerView(APIView):
    def get(self, request, user_id, *args, **kwargs):
        if user_id != None:
            ...
            if len(answers) != 0:
                serializer = serializers.GetUserActivitySerializer(answers, many=True)
                return Response(serializer.data)
            else:
                data = {
    ...
    def post(self, request, *args, **kwargs):
        serializer = serializers.PostAnswerSerializer(data=request.data, many=True)
        if serializer.is_valid():
            serializer.save()
            return Response(serializer.data)
        return Response(serializer.errors)